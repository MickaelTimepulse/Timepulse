  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const cleanedAthleteData = {
        ...athleteData,
        license_valid_until: athleteData.license_valid_until || null,
        license_type: athleteData.license_type || null,
        license_id: athleteData.license_id || null,
        license_club: athleteData.license_club || null,
        email: athleteData.email || null,
        phone: athleteData.phone || null,
        address_line1: athleteData.address_line1 || null,
        city: athleteData.city || null,
        postal_code: athleteData.postal_code || null,
        nationality: athleteData.nationality || 'FRA',
        is_anonymous: athleteData.is_anonymous,
      };

      const { data: athlete, error: athleteError } = await supabase
        .from('athletes')
        .insert([cleanedAthleteData])
        .select()
        .single();
      ...
      if (Object.keys(selectedOptions).length > 0) {
        ...
        const { error: optionsError } = await supabase
          ...
          .insert(registrationOptionsData);
