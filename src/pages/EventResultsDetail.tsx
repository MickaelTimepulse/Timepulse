...
export default function EventResultsDetail() {
  ...
  async function loadEventData() {
    try {
      const { data: eventData, error: eventError } = await Bolt Database
        .from('events')
      ...
      const { data: racesData, error: racesError } = await Bolt Database
        .from('races')
      ...
      if (statsError) {
        ...
        while (hasMore) {
          const { data: results, error: resultsError } = await Bolt Database
            .from('results')
            .select(`
              id,
              gender,
